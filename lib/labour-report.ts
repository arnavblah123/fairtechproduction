...
// Daily labour-calling report. Aggregates the day's calling activity pushed by the labour
...
export async function runLabourReport() {
  ...
  return {
    ok: true,
    emailed,
    // Say why nothing arrived, rather than failing silently.
    reason: emailed
      ? null
      : smtpConfigured
        ? "SMTP is configured but the send failed — check SMTP_USER/SMTP_PASS and the server logs"
        : "email not configured — set SMTP_USER and SMTP_PASS (Gmail app password) in Vercel",
    report,
  };
