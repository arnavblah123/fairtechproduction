...
async function main() {
  ...
  // --- One-time wipe #2 (guarded): owner requested another full redo of
  // Savli Unit-3 (Baroda) on 29 Jul — delete ALL its jobs again, cascades
  // take the stages, time logs, issues, tests and attachments.
  const savliWipe2Key = "patch.2026-07-29.delete-savli-jobs-2";
  if (!(await db.setting.findUnique({ where: { key: savliWipe2Key } }))) {
    const savli = await db.unit.findUnique({ where: { code: "SV3" } });
    if (savli) {
      const gone = await db.job.deleteMany({ where: { unitId: savli.id } });
      console.log(`Patch: deleted ${gone.count} Savli Unit-3 jobs for redo #2.`);
    }
    await db.setting.create({ data: { key: savliWipe2Key, value: "done" } });
  }
